...
mydata <- read.csv("Data/daily_temp.csv")
...
ts.logan <- df.logan |>
  ...
  select(DATE, TAVG) |>
  as_tsibble(index = DATE, key = c(TAVG)) |>
  arrange(DATE)
```
...
train <- as_tsibble(ts.logan[1:1460,], index = DATE, key = c(TAVG))
test <- as_tsibble(ts.logan[1461:1826,], index = DATE, key = c(TAVG))
...
lambda <- forecast::BoxCox.lambda(train$TAVG, method = c("guerrero"))

train$TAVG <- train$TAVG^lambda
train <- as_tsibble(train, index = DATE)
test$TAVG <- test$TAVG^lambda
...
fit <- train |> model(Naive = NAIVE(TAVG),
                      Drift = RW(TAVG ~ drift()),
                      SNaive = SNAIVE(TAVG, frequency = 4),
                      ETS = ETS(TAVG))
