...
I chose natural gas consumption in the US, from 2012 to 2021, which I obtained from the US Energy Information Administration. We will be examining 1973-2012 and observing how our forecasting models compare with the effects of the 2008 recession. The recession negatively impact many sectors of the economy, including transportation. Therefore, a drop-off in CO2 emissions in 2008 is expected. We will import the data, filter and organize it, and convert it into a time-series.
...
df.gas <- df.gas |> 
  filter(Description == "Natural Gas Consumption",
         substr(YYYYMM, 5, 6) != 13,
         between(substr(YYYYMM, 1, 4), "2012", "2021")) |>
  mutate(Year = substr(YYYYMM, 1, 4),
         Month = substr(YYYYMM, 5, 6),
         YearMonth = yearmonth(paste(Year, Month)),
         Consumption = as.numeric(Value)) |>
...
df.price <- df.henryhub |>
  ...
  filter(between(substr(Date, 5, 8), "2012", "2021")) |>
...
df.temp <- df.temp |>
  filter(between(substr(DATE, 1, 4), "2012", "2021")) |>
  mutate(YearMonth = yearmonth(DATE)) |>
  group_by(YearMonth) |> 
...
df.RD <- df.RD |> 
  filter(between(substr(TIME, 1, 4), "2012", "2021")) |>
...
Now we will plot the time-series that we will analyze: 2012 - 2022. 
```
```{r}

# Plotting the consumption time-series (2012 - 2021)
...
fit |> select(c(ETS)) |> forecast(h = 24) |> 
  autoplot(train) +
  autolayer(test) +
...
# Generating forecast plot
fit |> select(c(ARIMA)) |> forecast(h = 24) |> autoplot(train) +
  autolayer(test) +
