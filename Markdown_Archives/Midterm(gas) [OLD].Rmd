...
train <- as_tsibble(myts.dif[1:60,], 
                    index = YearMonth, 
                    key = c(Consumption, Price, Temp, Tech, Industry))
test <- as_tsibble(myts.dif[61:72,], 
                   index = YearMonth, 
                   key = c(Consumption, Price, Temp, Tech, Industry))
...
fit |> select(c(ARIMA)) |> forecast(h = 12) |> autoplot(train) +
  ...
  labs(title = "ARIMA Model Forecast",
       x = "Month",
       y = "Transformed Natural Gas Consumption") 
fit.lm |> select(c(LM)) |> forecast(new_data = test) |> autoplot(train) +
  autolayer(test) +
...
myf <- fit |> select(-c(LM)) |> forecast(h = 12)
myf.lm <- fit |> select(c(LM)) |> forecast(new_data = test)
myf <- rbind(myf, myf.lm)
